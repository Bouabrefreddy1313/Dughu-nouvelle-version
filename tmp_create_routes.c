...
const reportDir = 'src/app/api/comments/[id]/report'
fs.mkdirSync(reportDir, { recursive: true })

const reportContent = `import { NextRequest, NextResponse } from "next/server"
    ...
    for (const admin of admins) {
      void prisma.notification.create({
        data: {
          userId: admin.id,
          type: "report_comment",
          content: \`Un commentaire a été signalé: \${reason || "Mot non spécifié"}\`,
        },
      })
    }
...
fs.writeFileSync(path.join(reportDir, 'route.ts'), reportContent)
console.log('Report comment route created successfully')