...
* **Gestion des canaux privés et demandes d'adhésion** :
  ...
  - **Demandes d'adhésion reçues** :
    ...
    - **Traitement des demandes (`handleJoinRequest`)** : l'acceptation ou le refus cible l'endpoint officiel `POST /handleJoinRequest/:notificationId` (où `:notificationId` est l'identifiant `id` de la notification `JoinRequestNotification`), transmettant les headers `Content-Type: application/json` / `Accept: application/json` et le payload `{ user_id: adminUserId, accept: boolean }` (`true` pour accepter, `false` pour refuser, avec alias `accepte` de précaution). Les erreurs backend sont rigoureusement propagées sans masquage par un faux succès. L'action met à jour le statut, actualise `statut_canal_join_request`, notifie l'utilisateur par un toast et rafraîchit immédiatement la liste des demandes en attente, des demandes traitées et la liste des membres du canal.
...
* **Restauration automatique des conversations supprimées lors d'un nouvel échange** :
  - **Problématique résolue** : lorsqu'un utilisateur supprimait une conversation, celle-ci était masquée via `deletedFor: [userId]`. S'il écrivait à nouveau à la même personne ou recevait un message d'elle, la conversation restait masquée dans sa liste de discussions et n'apparaissait pas.
  - **Désoccultation automatique** : lors de l'envoi d'un nouveau message (`useSendMessage`) ou de l'ouverture d'un nouveau fil (`getOrCreateConversation`), les identifiants de l'expéditeur et du destinataire sont retirés de `deletedFor` (`arrayRemove(fromId, toId)`). La conversation réapparaît immédiatement et en temps réel dans la liste des discussions des deux participants.
  - **Préservation de la suppression d'historique (`deletedAt`)** : la date de suppression est conservée dans `deletedAt[userId]`. Les messages antérieurs à cette date restent définitivement masqués pour l'utilisateur qui a supprimé, lui offrant une discussion propre et fraîche à partir du nouveau message, sans réapparition intempestive de l'ancien historique supprimé.
  - **Résolution préventive (`MessagesPageClient`)** : détection immédiate de la conversation parent même lorsqu'elle était masquée, permettant une transition fluide sans latence ni état bloqué lors de l'envoi du premier message.
