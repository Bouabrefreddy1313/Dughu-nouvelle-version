  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        const data = await uploadProfileImage("avatar", avatarFormData)
        const resolvedAvatar = String(data?.avatar || data?.image || nextUser.avatar || "")
        if (!resolvedAvatar) throw new Error(String(data?.message || "Erreur lors de la mise à jour de l'avatar."))
      ...
      if (coverFile) {
        ...
        const data = await uploadProfileImage("cover", coverFormData)
        const resolvedCover = String(data?.cover || nextUser.cover || "")
        if (!resolvedCover) throw new Error(String(data?.message || "Erreur lors de la mise à jour de la couverture."))
