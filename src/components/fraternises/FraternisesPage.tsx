...
export default function FraternisesPage() {
  ...
  const loadFraternisesPosts = useCallback(
    async (page: number, reset = false) => {
      ...
      try {
        ...
        if (page > 1) setHasMore(false)
        console.error("loadFraternisesPosts error:", err)
        toast.error(
          isAbortError(err)
            ? "Le chargement a pris trop de temps. Veuillez réessayer."
            : "Impossible de charger les publications de vos amis fraternisés."
        )
      } finally {
        clearTimeout(timer)
      ...
      (entries) => {
        for (const entry of entries) {
          if (entry.isIntersecting && hasMore && !loading && !loadingMoreRef.current) {
            ...
            setPageNum(next)
            void loadFraternisesPosts(next, false)
          }
        }
      },
      { rootMargin: "300px" }
  ...
  }, [hasMore, loading, loadingMore, pageNum, loadFraternisesPosts])
  ...
  return (
    <MainLayout user={rawUser} active="fraternises">
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-[#1E1E1E] border-b border-gray-100 dark:border-white/10 sm:rounded-3xl sm:border sm:shadow-sm p-8 text-center my-6">
          ...
          <h2 className="text-lg font-bold text-[#1F2937] dark:text-[#F3F4F6] mb-1">
            Aucune publication de vos amis fraternisés
          </h2>
          <p className="text-sm text-[#65676B] dark:text-[#9CA3AF] max-w-md mx-auto mb-5">
            Dès que vous fraternisez avec d'autres membres et qu'ils publient du contenu, leurs actualités apparaîtront ici.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-3">
            <Button
              onClick={() => void loadFraternisesPosts(1, true)}
              variant="outline"
        ...
        <div className="space-y-4">
          {posts.map((post, postIndex) => (
            <Fragment key={`frat-feed-frag-${post.id}-${postIndex}`}>
              {post.isAkwaplayVideo && post.akwaplayData ? (
                <AkwaplayPostCard
                  key={`frat-card-${post.id}-${postIndex}`}
                  id={post.id}
                  ...
                  headerText="Akwaplay · Fraternisé"
                  onDismiss={() => {
                ...
                return (
                  <PostCard
                    key={`frat-post-${post.id}-${postIndex}`}
                    postId={post.id}
                    author={post.author}
                    currentUser={user}
