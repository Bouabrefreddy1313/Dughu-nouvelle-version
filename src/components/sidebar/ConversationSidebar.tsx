      ...
      <Card className="flex-1 flex flex-col rounded-[24px] rounded-r-none overflow-hidden shadow-[-8px_0_24px_rgba(0,0,0,0.06)] dark:shadow-[-8px_0_24px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-2 pb-4">
          {loading ? (
            ...
            <div className="px-5 py-10 text-center"><p className="text-sm text-red-600 dark:text-red-400">{error}</p><button onClick={() => void loadConversations(true)} className="mt-3 text-xs font-semibold text-[#A35A2A] dark:text-[#B46D1C]">Réessayer</button></div>
          ) : filtered.length === 0 ? (
